...
# Shows exactly what will be moved where

set -e

echo "🔍 INTUITIVE ORGANIZATION PREVIEW"
echo "================================="
echo ""
...
echo "Based on your 47,226+ files analysis, this will create:"
echo ""
...
if [ -d ~/active ]; then
    warning "~/active/ already exists - will merge"
else
    status "~/active/ will be created"
fi

if [ -d ~/tools ]; then
    warning "~/tools/ already exists - will merge"
else
    status "~/tools/ will be created"
fi

if [ -d ~/archive ]; then
    warning "~/archive/ already exists - will merge"
else
    status "~/archive/ will be created"
fi
...
if [ -d ~/pythons/projects ]; then
    proj_count=$(safe_count ~/pythons/projects)
    proj_size=$(dir_size ~/pythons/projects)
    move "~/pythons/projects/ ($proj_count files, $proj_size) → ~/active/projects/"
    active_total=$((active_total + proj_count))
fi

# AI research
if [ -d ~/workspace/ai_cli_research ]; then
    ai_count=$(safe_count ~/workspace/ai_cli_research)
    ai_size=$(dir_size ~/workspace/ai_cli_research)
    move "~/workspace/ai_cli_research/ ($ai_count files, $ai_size) → ~/active/ai-research/"
    active_total=$((active_total + ai_count))
fi

# Core utilities
if [ -f ~/pythons/avatar_utils.py ]; then
    move "~/pythons/avatar_utils.py → ~/active/avatar_utils.py"
    active_total=$((active_total + 1))
fi

if [ -f ~/pythons/MASTER_SNIPPETS.md ]; then
    move "~/pythons/MASTER_SNIPPETS.md → ~/active/MASTER_SNIPPETS.md"
    active_total=$((active_total + 1))
fi

# Active home files
home_active_count=0
for pattern in "avatararts_*.md" "gumroad*.txt" "AVATARARTS_*.md" "PYTHONS_*.md" "PYTHONS_*.py"; do
    for file in ~/$pattern; do
        if [ -f "$file" ] && [[ "$file" != ~/$pattern ]]; then
            filename=$(basename "$file")
            move "~/$filename → ~/active/$filename"
            home_active_count=$((home_active_count + 1))
        fi
    done
done

if [ $home_active_count -gt 0 ]; then
    move "$home_active_count active project files from home directory → ~/active/"
    active_total=$((active_total + home_active_count))
fi

count "TOTAL: ~/active/ will contain $active_total files"
...
# Python ecosystem
if [ -d ~/pythons ]; then
    py_count=$(safe_count ~/pythons)
    py_size=$(dir_size ~/pythons)
    move "~/pythons/ ($py_count files, $py_size) → ~/tools/python-ecosystem/"
    tools_total=$((tools_total + py_count))
fi

# AI configurations
if [ -d ~/.claude ]; then
    claude_count=$(safe_count ~/.claude)
    claude_size=$(dir_size ~/.claude)
    move "~/.claude/ ($claude_count files, $claude_size) → ~/tools/claude-config/"
    tools_total=$((tools_total + claude_count))
fi

if [ -d ~/.cursor ]; then
    cursor_count=$(safe_count ~/.cursor)
    cursor_size=$(dir_size ~/.cursor)
    move "~/.cursor/ ($cursor_count files, $cursor_size) → ~/tools/cursor-config/"
    tools_total=$((tools_total + cursor_count))
fi

if [ -d ~/.config/mcp ]; then
    mcp_count=$(safe_count ~/.config/mcp)
    move "~/.config/mcp/ ($mcp_count files) → ~/tools/mcp-config/"
    tools_total=$((tools_total + mcp_count))
fi

# Scripts
if [ -d ~/scripts ]; then
    scripts_count=$(safe_count ~/scripts)
    scripts_size=$(dir_size ~/scripts)
    move "~/scripts/ ($scripts_count files, $scripts_size) → ~/tools/shell-scripts/"
    tools_total=$((tools_count + scripts_count))
fi

# Tool scripts from home
home_tools_count=0
for pattern in "update_music_inventory.py" "compare_hashes*.py" "find_functional_dupes.py" "deep_logic_audit.py" "generate_redundancy_map.py" "extract_redundant_code.py" "comment_zshrc_sections.py" "create_*.sh" "consolidation_*.sh" "intuitive_*.sh" "simple_*.sh" "implement_*.sh"; do
    for file in ~/$pattern; do
        if [ -f "$file" ] && [[ "$file" != ~/create_comprehensive_index.sh ]] && [[ "$file" != ~/$pattern ]]; then
            filename=$(basename "$file")
            move "~/$filename → ~/tools/$filename"
            home_tools_count=$((home_tools_count + 1))
        fi
    done
done

if [ $home_tools_count -gt 0 ]; then
    move "$home_tools_count development utility scripts from home → ~/tools/"
    tools_total=$((tools_total + home_tools_count))
fi

count "TOTAL: ~/tools/ will contain $tools_total files"
...
if [ -d ~/pythons/archives ]; then
    arch_count=$(safe_count ~/pythons/archives)
    arch_size=$(dir_size ~/pythons/archives)
    move "~/pythons/archives/ ($arch_count files, $arch_size) → ~/archive/old-archives/"
    archive_total=$((archive_total + arch_count))
fi

# Old home files
home_archive_count=0
for pattern in "error.*" "folder by folder if needed.txt" "remote_down.txt" "local_down.txt" "BATCH*.txt" "ETSY*.txt" "PROGRESS_REPORT.txt" "ZIP_ORGANIZATION_FINAL.txt" "cursor-agent.txt" "gemini*.txt" "gemini*.md" "QWEN.md" "AI_Chats" "Archives" "NEXT_ORGANIZATION_IDEAS.md" "REVIEW_BEFORE_MOVING.txt" "batch*.txt" "comparison_batches.txt" "2T-Xx_inventory_report.txt" "FULL_HOME_REVIEW*.md" "gemini-conversation-*.json" "AVATARARTS_CONSOLIDATION_CHANGELOG.md"; do
    for file in ~/$pattern; do
        if [ -f "$file" ] && [[ "$file" != ~/$pattern ]]; then
            filename=$(basename "$file")
            move "~/$filename → ~/archive/old-home-files/"
            home_archive_count=$((home_archive_count + 1))
        fi
    done
done

if [ $home_archive_count -gt 0 ]; then
    move "$home_archive_count old files from home directory → ~/archive/old-home-files/"
    archive_total=$((archive_total + home_archive_count))
fi

count "TOTAL: ~/archive/ will contain $archive_total files"

echo ""
preview "FINAL STRUCTURE SUMMARY"
echo "========================"

grand_total=$((active_total + tools_total + archive_total))
