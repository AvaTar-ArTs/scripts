...
def generate_dry_run_csv(directories, csv_path):
    ...
    excluded_patterns = [
        r"^\..*", r".*/venv/.*", r".*/\.venv/.*", r".*/lib/.*", r".*/\.lib/.*",
        r".*/my_global_venv/.*", r".*/simplegallery/.*", r".*/avatararts/.*",
        r".*/github/.*", r".*/Documents/gitHub/.*", r".*/\.my_global_venv/.*",
        r".*/node/.*", r".*/miniconda3/.*", r".*/env/.*", r".*/\.env/.*",
...
if __name__ == "__main__":
    ...
    while True:
        if last_directory:
            ...
            source_directory = input("Please enter a source directory to scan for document files: ").strip()
