...
maintain-avatararts-ai() {
    ...
    if [[ -d "$HOME/AVATARARTS" ]]; then
        ...
        for subdir in "${large_dirs[@]}"; do
            if [[ -d "$subdir" ]]; then
                # Use printf %q to safely quote the subdirectory path for use in a command string
                local quoted_subdir=$(printf %q "$subdir")
                avatararts_commands+=("find ${quoted_subdir} -name '*.tmp' -type f -delete 2>/dev/null;
                                      find ${quoted_subdir} -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null;
                                      find ${quoted_subdir} -name '*.log' -type f -mtime +7 -delete 2>/dev/null")
            fi
        done
    ...
    if [[ -d "$HOME/.harbor" ]]; then
        ...
        for harbor_dir in "${harbor_components[@]}"; do
            if [[ -d "$harbor_dir" ]]; then
                local quoted_harbor_dir=$(printf %q "$harbor_dir")
                harbor_commands+=("find ${quoted_harbor_dir} -name '*.tmp' -type f -delete 2>/dev/null;
                                  find ${quoted_harbor_dir} -name '*.log' -type f -mtime +7 -delete 2>/dev/null;
                                  find ${quoted_harbor_dir} -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null")
            fi
        done
...
maintain-intellihub-ai() {
    ...
    if [[ -d "$HOME/IntelliHub" ]]; then
        ...
        for subdir in "${intellihub_dirs[@]}"; do
            if [[ -d "$subdir" ]]; then
                local quoted_subdir=$(printf %q "$subdir")
                intellihub_commands+=("find ${quoted_subdir} -name '*.tmp' -type f -delete 2>/dev/null;
                                      find ${quoted_subdir} -name '*.log' -type f -mtime +7 -delete 2>/dev/null;
                                      find ${quoted_subdir} -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null")
            fi
        done
...
maintain-git-repos-ai() {
    ...
    if command_exists git; then
        ...
        for repo_path in "${repos_to_update[@]}"; do
            local quoted_repo_path=$(printf %q "$repo_path")
            git_commands+=("(cd ${quoted_repo_path} && git fetch origin 2>/dev/null && git remote prune origin 2>/dev/null && echo 'Updated $(basename "$repo_path")') 2>/dev/null")
        done
...
maintain-python-envs-ai() {
    ...
    for env_dir in "${large_py_envs[@]}"; do
        if [[ -d "$env_dir" ]]; then
            local quoted_env_dir=$(printf %q "$env_dir")
            py_commands+=("find ${quoted_env_dir} -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null;
                          find ${quoted_env_dir} -name '*.pyc' -type f -delete 2>/dev/null;
                          find ${quoted_env_dir} -name '*.pyo' -type f -delete 2>/dev/null")
        fi
    done
...
maintain-ai-tools-ai() {
    ...
    for ai_dir in "${active_ai_dirs[@]}"; do
        if [[ -d "$ai_dir" ]]; then
            local quoted_ai_dir=$(printf %q "$ai_dir")
            ai_commands+=("find ${quoted_ai_dir} -name '*.tmp' -type f -delete 2>/dev/null;
                           find ${quoted_ai_dir} -name '*.log' -type f -mtime +7 -delete 2>/dev/null;
                           find ${quoted_ai_dir} -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null")
        fi
    done
...
comprehensive-cleanup-ai() {
    ...
    if [[ -d ~/Library/Logs ]]; then
        ...
        if [[ -n "$log_size" && "$log_size" -gt 100000000 ]]; then  # 100MB threshold
            local quoted_log_dir=$(printf %q "$HOME/Library/Logs")
            cleanup_commands+=("find ${quoted_log_dir} -name '*.log' -type f -mtime +7 -delete 2>/dev/null")
        fi
    fi
    ...
    if [[ -d ~/Library/Caches ]]; then
        ...
        if [[ -n "$cache_size" && "$cache_size" -gt 500000000 ]]; then  # 500MB threshold
            local quoted_cache_dir=$(printf %q "$HOME/Library/Caches")
            cleanup_commands+=("find ${quoted_cache_dir} -name '*.cache' -type f -mtime +7 -delete 2>/dev/null")
        fi
    fi
    ...
    if [[ -d ~/.cache ]]; then
        ...
        if [[ -n "$user_cache_size" && "$user_cache_size" -gt 100000000 ]]; then  # 100MB threshold
            local quoted_user_cache_dir=$(printf %q "$HOME/.cache")
            cleanup_commands+=("find ${quoted_user_cache_dir} -name '*.tmp' -type f -delete 2>/dev/null")
        fi
    fi
    ...
    if [[ -d ~/.npm/_cacache ]]; then
        ...
        if [[ -n "$npm_cache_size" && "$npm_cache_size" -gt 100000000 ]]; then  # 100MB threshold
            local quoted_npm_cache_dir=$(printf %q "$HOME/.npm/_cacache")
            cleanup_commands+=("find ${quoted_npm_cache_dir} -name '*.tmp' -type f -delete 2>/dev/null")
        fi
    fi
    
    # Add general cleanup commands
    # These are hardcoded paths so printf %q is not strictly needed but adds consistency
    local quoted_logs_dir=$(printf %q "$HOME/logs")
    cleanup_commands+=(
        "find ${quoted_logs_dir} -name '*.log' -type f -mtime +30 -delete 2>/dev/null"
        "find ${quoted_logs_dir} -name '*.log.*' -type f -mtime +30 -delete 2>/dev/null"
    )
