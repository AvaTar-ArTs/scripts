...
# Advanced System Maintenance Pro v5 - Guided Mode
# Provides detailed information about operations and asks for confirmation
# before performing each major operation
...
LOGFILE="${LOGDIR}/v5-guided-maintenance-log-${TIMESTAMP}.log"
BACKUP_DIR="${HOME}/backups/maintenance-v5-guided-${TIMESTAMP}"
...
confirm_operation() {
    local operation=$1
    local details=$2
    print_status $YELLOW "⚠️  ABOUT TO PERFORM: $operation"
    print_status $CYAN "   DETAILS: $details"
    echo
    local response
    while true; do
        read -p "   Proceed with this operation? (y/n/skip/all) " -n 1 -r response
        echo
        case $response in
            [yY]) 
                log_message "INFO" "User confirmed operation: $operation"
                return 0 ;;
            [aA]) 
                log_message "INFO" "User selected 'all' - bypassing future confirmations"
                return 2 ;;  # Return 2 to indicate "all" mode
            [sS]) 
                log_message "INFO" "User skipped operation: $operation"
                return 1 ;;
            [nN]) 
                print_status $RED "   ❌ SKIPPED: $operation"
                log_message "INFO" "User declined operation: $operation"
                return 1 ;;
            *) echo "Please answer y (yes), n (no), s (skip), or a (all)" ;;
        esac
    done
}
...
# Function to prompt yes/no (used for non-critical confirmations)
prompt_yes_no() {
    local question=$1
    local response
    while true; do
...
# Global variable to track if user selected "all" mode
ALL_MODE=false
...
maintain-avatararts-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AVATARARTS Ecosystem Maintenance" "Will analyze and clean AVATARARTS directories, create backups, and optimize ecosystem components")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping AVATARARTS ecosystem maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "AVATARARTS maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
    fi
...
maintain-harbor-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "Harbor Ecosystem Maintenance" "Will analyze and clean Harbor components, create backups, and optimize ecosystem components")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping Harbor ecosystem maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "Harbor maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
maintain-intellihub-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "IntelliHub Ecosystem Maintenance" "Will analyze and clean IntelliHub components, create backups, and optimize ecosystem components")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping IntelliHub ecosystem maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "IntelliHub maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
maintain-git-repos-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "Git Repository Management" "Will scan for Git repositories and update those without uncommitted changes")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping Git repository management"
            ((stats[operations_failed]++))
            log_message "INFO" "Git maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
maintain-python-envs-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "Python Environments Maintenance" "Will analyze Python environments and clean cache files in large environments")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping Python environments maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "Python environments maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
maintain-ai-tools-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI/ML Tools Maintenance" "Will clean temporary files and caches in AI/ML tool directories")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping AI/ML tools maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "AI/ML tools maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
maintain-dev-tools-smart() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "Development Tools Maintenance" "Will clean caches for npm, yarn, pip, and docker if they exceed size thresholds")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping development tools maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "Development tools maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
update-brew-smart() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "Smart Homebrew Updates" "Will update Homebrew, upgrade packages, and clean up old versions")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping Homebrew updates"
            ((stats[operations_failed]++))
            log_message "INFO" "Homebrew updates skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
update-conda-smart() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        ...
        if command_exists mamba; then
            confirmed=$(confirm_operation "Smart Mamba Updates" "Will update Mamba, packages, and clean cache")
        elif command_exists conda; then
            confirmed=$(confirm_operation "Smart Conda Updates" "Will update Conda, packages, and clean cache")
        else
        ...
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping Conda/Mamba updates"
            ((stats[operations_failed]++))
            log_message "INFO" "Conda/Mamba updates skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
clear-system-caches-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI-Prioritized System Caches Clearing" "Will analyze and clear large system cache directories")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping system caches clearing"
            ((stats[operations_failed]++))
            log_message "INFO" "System caches clearing skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
analyze-large-dirs-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI-Driven Large Directories Analysis" "Will analyze your largest directories and provide cleanup recommendations")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping large directories analysis"
            ((stats[operations_failed]++))
            log_message "INFO" "Large directories analysis skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
maintain-env-configs-smart() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "Smart Environment Configurations Maintenance" "Will check and backup important configuration files")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping environment configurations maintenance"
            ((stats[operations_failed]++))
            log_message "INFO" "Environment configurations maintenance skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
manage-containers-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        if command_exists docker; then
            local confirmed=-1
            confirmed=$(confirm_operation "AI-Enhanced Container Management" "Will analyze and clean Docker containers, images, and volumes")
            if [[ $confirmed -eq 1 ]]; then
                print_status $YELLOW "      ⊘ Skipping container management"
                ((stats[operations_failed]++))
                log_message "INFO" "Container management skipped by user"
                return
            elif [[ $confirmed -eq 2 ]]; then
                ALL_MODE=true
            fi
...
security-audit-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI-Enhanced Security Audit" "Will perform security checks on file permissions and system logs")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping security audit"
            ((stats[operations_failed]++))
            log_message "INFO" "Security audit skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
comprehensive-cleanup-ai() {
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI-Driven Comprehensive Cleanup" "Will perform comprehensive cleanup of temporary files, logs, and caches")
        if [[ $confirmed -eq 1 ]]; then
            print_status $YELLOW "      ⊘ Skipping comprehensive cleanup"
            ((stats[operations_failed]++))
            log_message "INFO" "Comprehensive cleanup skipped by user"
            return
        elif [[ $confirmed -eq 2 ]]; then
            ALL_MODE=true
        fi
...
{
  "system_id": "$(hostname)",
  "first_maintenance_date": "$(date +%Y-%m-%d)",
  "maintenances": [
    $(cat $stats_file)
  ]
}
...
maintenance-all() {
    print_status $BOLD $PURPLE "🚀 ADVANCED SYSTEM MAINTENANCE PRO V5 - GUIDED MODE"
    print_status $BOLD $PURPLE "   AI-Driven Ecosystem Management with User Confirmation"
    print_status $CYAN "=================================================================="
    print_status $GREEN "Starting guided AI-driven system maintenance with user confirmation..."
    print_status $CYAN "=================================================================="
    echo ""

    print_status $YELLOW "⚠️  GUIDED MODE: You will be prompted before each major operation ⚠️"
    echo ""
    ...
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI Analysis and Predictions" "Will perform AI analysis and generate predictive maintenance insights")
        if [[ $confirmed -eq 0 ]] || [[ $confirmed -eq 2 ]]; then
            if [[ $confirmed -eq 2 ]]; then
                ALL_MODE=true
            fi
            perform_ai_analysis
    ...
    if [[ "$DEFAULT_CLOUD_SYNC_ENABLED" == true ]]; then
        if [[ "$ALL_MODE" == false ]]; then
            local confirmed=-1
            confirmed=$(confirm_operation "Cloud Synchronization" "Will sync maintenance data to cloud storage")
            if [[ $confirmed -eq 0 ]] || [[ $confirmed -eq 2 ]]; then
                if [[ $confirmed -eq 2 ]]; then
                    ALL_MODE=true
                fi
...
# Perform AI analysis (called from maintenance-all if confirmed)
perform_ai_analysis() {
    if [[ "$DEFAULT_AI_ANALYSIS_ENABLED" == true ]]; then
...
# Generate predictions for future maintenance
generate_predictions() {
    if [[ "$DEFAULT_PREDICTIVE_MAINTENANCE" == true ]]; then
...
maintenance-quick() {
    print_status $BOLD $PURPLE "⚡ QUICK AI-ENHANCED MAINTENANCE - GUIDED MODE"
    print_status $CYAN "=================================="
    print_status $GREEN "Running quick maintenance with AI insights and user confirmation..."
    echo ""
    ...
    # Perform quick AI analysis if confirmed
    if [[ "$ALL_MODE" == false ]]; then
        local confirmed=-1
        confirmed=$(confirm_operation "AI Analysis" "Will perform quick AI analysis")
        if [[ $confirmed -eq 0 ]] || [[ $confirmed -eq 2 ]]; then
            if [[ $confirmed -eq 2 ]]; then
                ALL_MODE=true
            fi
...
# Help function
show_help() {
    print_status $BOLD $PURPLE "ADVANCED SYSTEM MAINTENANCE PRO V5 - GUIDED MODE - HELP"
    print_status $CYAN "============================================="
    echo ""
    print_status $GREEN "Guided Mode Features:"
    echo "  • Detailed information about each operation"
    echo "  • User confirmation before major operations"
    echo "  • Options to skip, proceed, or approve all"
    echo "  • AI-powered analysis and recommendations"
    echo "  • Smart resource utilization"
    echo ""
    print_status $GREEN "Available functions:"
    echo "  maintenance-all           - Full AI-driven maintenance with confirmations"
    echo "  maintenance-quick         - Quick AI-enhanced maintenance with confirmations"
    echo "  config-show              - Show current configuration"
    echo ""
    print_status $CYAN "Usage:"
    echo "  ./advanced-system-maintenance-v5-guided.sh [function_name]"
    echo "  ./advanced-system-maintenance-v5-guided.sh maintenance-all"
    echo "  ./advanced-system-maintenance-v5-guided.sh maintenance-quick"
    echo ""
    print_status $CYAN "Confirmation Options:"
    echo "  y - Yes, proceed with this operation"
    echo "  n - No, skip this operation"
    echo "  s - Skip, same as 'n'"
    echo "  a - All, proceed with this and all future operations without asking"
    echo ""
}
