...
# Recursive Content-Aware Rename Script
# Renames ALL directories and files recursively from /Users/steven/AvaTarArTs

BASE_PATH="/Users/steven/AvaTarArTs"

echo "🎯 RECURSIVE CONTENT-AWARE RENAMING"
echo "===================================="
    ...
    for dir in "$current_path"/*; do
        if [ -d "$dir" ]; then
            ...
            if [ "$dir_name" != "$new_name" ]; then
                echo "📁 [$depth] Renaming: $dir_name → $new_name"
                result=$(rename_directory_and_files "$dir_name" "$new_name" "$current_path")
                case $? in
                    0) echo "   ✅ Success" ;;
                    1) echo "   ❌ Failed to rename directory" ;;
                    2) echo "   ⚠️  Directory not found" ;;
                esac
            fi
            
            # Recursively process the directory (use new name if renamed)
            if [ "$dir_name" != "$new_name" ]; then
                process_directory "$current_path/$new_name" $((depth + 1))
            else
                process_directory "$dir" $((depth + 1))
            fi
        fi
    done
}

echo "📊 Starting recursive processing of all directories..."
echo "📁 Total directories to process: $(find "$BASE_PATH" -type d | wc -l)"
echo ""
...
echo ""
read -p "❓ Do you want to proceed with recursive renaming of ALL directories? (y/N): " confirm
...
if [[ $confirm =~ ^[Yy]$ ]]; then
    ...
    process_directory "$BASE_PATH" 0
    
    echo ""
    echo "🏁 Recursive renaming completed!"
else
