...
def generate_csv(directories, csv_path):
    rows = []
    excluded_patterns = [
        r"^\..*", r".*/venv/.*", r".*/\.venv/.*", r".*/my_global_venv/.*", r".*/simplegallery/.*",
        r".*/avatararts/.*", r".*/github/.*", r".*/Documents/gitHub/.*", r".*/\.my_global_venv/.*",
        r".*/node/.*", r".*/Movies/capcut/.*", r".*/miniconda3/.*", r".*/Movies/movavi/.*",
        r".*/env/.*", r".*/\.env/.*", r".*/Library/.*", r".*/\.config/.*", r".*/\.spicetify/.*",
        r".*/\.gem/.*", r".*/\.zprofile/.*", r"^.*\/\..*",
    ]

    file_types = {
        ".jpg": "Image", ".jpeg": "Image", ".png": "Image", ".bmp": "Image",
        ".gif": "Image", ".tiff": "Image",
    }
...
def write_csv(csv_path, rows):
    with open(csv_path, "w", newline="") as csvfile:
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({"Filename": row[0], "File Size": row[1], "Creation Date": row[2], "Width": row[3], "Height": row[4], "DPI_X": row[5], "DPI_Y": row[6], "Original Path": row[7]})
